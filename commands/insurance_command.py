...
class InsuranceModal(discord.ui.Modal, title="แบบฟอร์มออกบัตรประกันสุขภาพ"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed = discord.Embed(
                title="💳 บัตรประจำตัว Pracharuk Care",
            ...
            embed.add_field(
                name="⚤ เพศ",
                value=self.gender_input.value,
                inline=True
            )

            embed.add_field(
                name="📞 เบอร์โทร",
                value=self.phone_input.value,
                inline=True
            )

            embed.add_field(
                name="📅 วันที่ออกบัตร",
                value=f"{i_date} | {i_time}",
                inline=False
            )

            embed.add_field(
                name="⏳ วันที่หมดอายุ",
                value=f"{e_date} | {e_time}",
                inline=False
            )

            embed.set_footer(
                text=f"ผู้ออกบัตร: {interaction.user.display_name}"
            )
...
@app_commands.command(
    name="ออกบัตรประกัน",
    description="ออกบัตรประกันสุขภาพพร้อมรูปถ่าย"
)
@app_commands.describe(
    image="แนบรูปถ่ายหน้าตรง"
)
async def insurance(interaction: discord.Interaction, image: discord.Attachment):
    ...
    if not image.content_type or not image.content_type.startswith("image/"):
        ...
        return await interaction.response.send_message(
            "❌ โปรดแนบไฟล์รูปภาพเท่านั้น",
            ephemeral=True
        )

    view = DurationView(image)

    await interaction.response.send_message(
        "เลือกระยะเวลาบัตร แล้วกดยืนยัน",
        view=view,
        ephemeral=True
    )